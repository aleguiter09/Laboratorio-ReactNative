...
const Detalle = ({route: {params}, ...props}) => {
  ...
  const {obtenerCategoriasDelProducto, obtenerCompradoresDelProducto} = useContext(StoreContext);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text category="h5">Categorias:</Text>
      <FlatList
        data={categorias}
        horizontal
        renderItem={({item}) => (
      ...
      />
      <Text category="h5">Compradores:</Text>
      <FlatList
        data={compradores}
        renderItem={({item}) => (
          <View>
